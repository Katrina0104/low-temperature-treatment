    ...
    public static class OcclusionMetallicRoughnessConverter
    {
        ...
        private static Material Exporter
        {
            get
            {
                if (_exporter == null)
                {
                    // 嘗試查找 Hidden/UniGLTF/StandardMapExporter Shader
                    Shader shader = Shader.Find("Hidden/UniGLTF/StandardMapExporter");
                    if (shader == null)
                    {
                        // 如果找不到，使用 Unity 預設的 Standard Shader 作為回退選項
                        Debug.LogWarning("Shader 'Hidden/UniGLTF/StandardMapExporter' not found. Using 'Standard' shader as fallback.");
                        shader = Shader.Find("Standard");
                    }
                    _exporter = new Material(shader);
                }
                return _exporter;
            }
        }
